...
function GetEjecutor(url,id,id1){
	url=url+id1
	...
	getAjax.onreadystatechange=function() {
		if (getAjax.readyState==4)	{
			//alert(getAjax.responseText);
			mostrar(getAjax.responseText,id,id1);
		}
	}
	getAjax.send(null);
...
function mostrar(dato,id,id1){
	campo=dato.split("*@*#*");// alert(campo[5]+"-"+campo[2]);
	document.getElementById('siglas').value=campo[6]+"-"+campo[2];
	document.getElementById('coordinador').value=campo[3];
	document.getElementById('email').value=campo[4];
