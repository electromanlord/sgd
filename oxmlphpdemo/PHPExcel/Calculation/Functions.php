...
//			$ta = 2 / pi();
	...
	public static function VERSION() {
		return 'PHPExcel 1.6.7, 2009-04-22';
	}
	...
	private static $conversionUnits = array( 'g'		=> array(	'Group'	=> 'Mass',			'Unit Name'	=> 'Gram',						'AllowPrefix'	=> True		),
	...
	private static $conversionMultipliers = array(	'E'	=> array(	'multiplier'	=> 1E18,	'name'	=> 'exa'	),
													...
													'e'	=> array(	'multiplier'	=> 1E1,		'name'	=> 'dekao'	),
	...
	private static $unitConversions = array(	'Mass'		=> array(	'g'		=> array(	'g'		=> 1.0,
																		...
																		'lbm'	=> array(	'g'		=> 4.53592309748811E+02,
...
//			$fRet = sqrt(0.636619772 / $fNum) * sin($fNum - 2.356194491);
	...
	public static function DOLLARDE($fractional_dollar = Null, $fraction = 0) {
		...
		return floor($fractional_dollar) + ((($fractional_dollar - floor($fractional_dollar)) * 100) / $fraction);
	}
	...
	public static function DOLLARFR($decimal_dollar = Null, $fraction = 0) {
		...
		return floor($decimal_dollar) + ((($decimal_dollar - floor($decimal_dollar)) * $fraction) / 100);
	}
	...
	*/
	...
	public static function DB($cost, $salvage, $life, $period, $month=12) {
		...
		if ((is_numeric($cost)) && (is_numeric($salvage)) && (is_numeric($life)) && (is_numeric($period)) && (is_numeric($month))) {
			if (($cost <= 0) || (($salvage / $cost) < 0) || ($life <= 0) || ($period < 1)  || ($month < 1)  || ($period > $life)) {
				return self::$_errorCodes['num'];
			}
	...
	private static function _daysPerYear($year,$basis) {
		switch ($basis) {
			...
			case 3 :
				...
				break;
			case 1 :
				if (self::isLeapYear(self::YEAR($year))) {
					$daysPerYear = 366;
				} else {
					$daysPerYear = 365;
				}
	...
	public static function ACCRINTM($issue, $settlement, $rate, $par=1000, $basis=0) {
		...
		$par		= (float) self::flattenSingleValue($par);
	...
	public static function DISC($settlement, $maturity, $price, $redemption, $basis=0) {
		...
		if ((is_numeric($price)) && (is_numeric($redemption)) && (is_numeric($basis))) {
			...
			return (($redemption - $price) / $redemption) * ($daysPerYear / $daysBetweenSettlementAndMaturity);
		}
		return self::$_errorCodes['value'];
	}	//	function DB()
	...
	public static function PRICEDISC($settlement, $maturity, $discount, $redemption, $basis=0) {
		...
		if ((is_numeric($discount)) && (is_numeric($redemption)) && (is_numeric($basis))) {
			...
			return $redemption - $discount * $redemption * ($daysBetweenSettlementAndMaturity / $daysPerYear);
		}
		return self::$_errorCodes['value'];
	}	//	function DB()
	...
	public static function RECEIVED($settlement, $maturity, $investment, $discount, $basis=0) {
		...
		if ((is_numeric($investment)) && (is_numeric($discount)) && (is_numeric($basis))) {
			...
			return $investment / ( 1 - ($discount * ($daysBetweenSettlementAndMaturity / $daysPerYear)));
		}
		return self::$_errorCodes['value'];
	}	//	function RECEIVED()
	...
	public static function INTRATE($settlement, $maturity, $investment, $redemption, $basis=0) {
		...
		if ((is_numeric($investment)) && (is_numeric($redemption)) && (is_numeric($basis))) {
			...
			return (($redemption - $investment) / $investment) * ($daysPerYear / $daysBetweenSettlementAndMaturity);
		}
		return self::$_errorCodes['value'];
	}	//	function INTRATE()
	...
	public static function INDEX($range_array,$row_num=null,$column_num=null) {
		...
		print_r($range_array);
		echo '<br />';
		echo '$column_num='.$column_num.'; $row_num='.$row_num.'<br />';
		/**/
