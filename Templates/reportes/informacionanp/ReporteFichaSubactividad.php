...
$_SESSION['anp']['idanp']=$anpid;
	if ($_POST[id_var]){
		$_SESSION['id_ff_anp']=array();
		$_SESSION['id_ff_anp'][0]=$_POST[id_var];
		$id=$_POST[id_var];
		$numpags=1;
		$pga=1;
	}
	...
	elseif($_POST[id_var]) $id=$_POST[id_var]; 
	else $id=$_GET[id];
	...
	$sql_nom="SELECT * FROM anp WHERE id_anp='".$row['id_anp']."'";		
	$query_nom= new Consulta($sql_nom);
	$row_nom=$query_nom->ConsultaVerRegistro();
...
if(!$moneda || $moneda=="SOLES"){
	...
	$Q_axo=new Consulta("Select tipo_cambio FROM axo_poa WHERE id_axo_poa='".$_SESSION['inrena_4']['2']."'");
...
<table align="center" bgcolor="#FFFFFF" width="100%" id="ficha">
  <!--DWLayoutTable-->
	<tr><td height="26" colspan="6" align="center">
		<div align="center" style=" azimuth:center; font: bold 11px tahoma"><b>Plan Operativo -<?=axo($_SESSION['inrena_4'][2])?></b></div>
	</td></tr>
	<tr><td height="26" colspan="6" align="center"><div align="center"><b>Ficha de Tarea </b></div>
	</td></tr>
	<tr><td height="20" colspan="6" align="center"><div align="center"><b>
		<?=$row_nom['nombre_anp']?></b></div>
	</td></tr>
	<tr>
    ...
    <tr class="titulo" bgcolor="#999999">
		<td height="26" align="center" class="titulo">C&oacute;digo </td>
		<td colspan="3" class="titulo" align="center">Nombre</td>
		<td colspan="2" align="center" class="titulo">Fte. Fto. - Ejec. </td>
	</tr>
	<tr>
		<td height="26" valign="middle" class="descripcion"><? 
			...
			echo $rws[1]."."; ?></td>
		<td valign="middle" class="descripcion"><div align="left">Estrategia General :</div></td>
		<td colspan="2" valign="middle" class="descripcion"><?=$rws[0]?></td>
		<td colspan="2" align="center" class="descripcion"><?=$row_ff['siglas_ff']?></td>
	</tr>
	<tr>
		<td height="26" valign="middle" class="descripcion">
		...
		<td valign="middle" class="descripcion"><div align="left">Estrategia Espec&iacute;fica  : </div></td>					
	    <td colspan="2" valign="middle" class="descripcion"><?=$rw[0]?></td>
	    <td valign="top" class="titulo">Monto</td>
	    <td valign="top" class="titulo">Moneda</td>
	</tr>
	<tr>
		<td height="26" valign="middle" class="descripcion"><?=$rws[1].".".$rw[1].".".$row[nro_asignacion]?></td>
		<td valign="middle" class="descripcion"><div align="left">Tarea : </div></td>					
	    <td colspan="2" valign="middle"class="descripcion"><?=$row['nom_tarea']?></td>
	    <td valign="top" class="descripcion" align="center"><?=number_format(($row_monto[monto]/$tc),2)?></td>
    <td align="center" valign="top" class="descripcion">
	<? if($_GET[pdf]!='ok'){ ?>
		<a href="<?=$PHP_SELF?>?moneda=<?=$money?>&amp;id=<?=$id?>&amp;pga=<?=$pga?>"> <?=$moneda?> </a><? } else echo $moneda;?></td>
	</tr>
	<tr>
		<td height="26" colspan="3" valign="middle" bgcolor="#999999" class="titulo">Descripci&oacute;n de Tarea </td>
	    <td valign="middle" class="titulo">Unidad de Medida </td>
		<td colspan="2" valign="top" class="titulo">Meta</td>
	</tr>
	<tr bgcolor="#eeeeee">
		<td height="26" colspan="3" valign="middle" class="descripcion"><?=$row['desc_asig_ff_anp_obj']?></td>
	<td align="center" valign="middle"><?=$row_sa['medio_verificacion_tarea']?></td>
	...
	<tr class="titulo" bgcolor="#999999">
		<td height="26" colspan="4" class="titulo">Responsable</td>
		<td colspan="2" class="titulo">Cargo</td>
	</tr>
	<tr bgcolor="#eeeeee">
		<td height="26" colspan="4" class="descripcion"><?
			$sq_personal=new Consulta("SELECT * FROM cargo_anp
				Inner Join cargo_personal_anp ON cargo_anp.id_cargo_anp = cargo_personal_anp.id_cargo_anp
				Inner Join personal_anp ON personal_anp.id_personal_anp = cargo_personal_anp.id_personal_anp 
			WHERE personal_anp.id_personal_anp='".$row['id_personal_anp']."' 
			order by nombre_personal_anp asc,apellidos_personal_anp asc");
			
					$row_per=$sq_personal->ConsultaVerRegistro();
					echo $row_per['nombre_personal_anp']." ".$row_per['apellidos_personal_anp'];
		?></td>
		<td colspan="2" align="center" class="descripcion"> <?=$row_per['nombre_cargo_anp']?></td>
	</tr>
</table>
