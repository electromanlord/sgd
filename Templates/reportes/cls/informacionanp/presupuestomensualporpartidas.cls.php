...
////$idmes->SqlSelect($_SESSION['idmes']);


}			
function mostrar_ff_chk($fuente=''){

		$funcjs=" mostrar_presupuestomensualporpartidas";
		Verkff($funcjs,$this);	 
...
function setsql($sql=''){
	...
	function ListarPartidas(){
		...
		$sqlPartidas=$this->set_sql("pp.id_partida","$sqlFfts AND m.id_mes='".$_SESSION[idmes]."'","pp.id_partida","partida.codigo_partida","partida");
		//die ($sqlPartidas);
	...
	function mostrar_Presupuesto_tarea(){
		$gsqlp=$this->SqlFts;
		$sqlg=$gsqlp;
		$this->setsql($gsqlp);
			  ...
			  //OBJ Estrat ///////////////////////////////////////////////////////////////////////
				//die ()
				$sqlprg=$this->set_sql("oe.*,m.*","$gsqlp  AND m.id_mes='".$_SESSION[idmes]."'", "oe.id_objetivo_estrategico","codigo_objetivo_estrategico");
				...
				while($row_prg=$query->ConsultaVerRegistro()){
					...
					$prg[total_monto]=$row_prg[total_monto];
						$gsqlsp=" $gsqlp  AND m.id_mes='".$_SESSION[idmes]."' and oe.id_objetivo_estrategico='".$row_prg[id_objetivo_estrategico]."'";
						$this->setsql($gsqlsp);
						
						$this->registro_programa($row_prg,$ftefto);
					
						//OBJETIVO ESPECIFICO///////////////////////////////////////////////////////////////////////
					$sql_sb=$this->set_sql("aoesp.*,m.*","$gsqlsp and m.id_mes='".$_SESSION[idmes]."'","aoesp.id_anp_objetivo_especifico ","codigo_objetivo_especifico");
						$query_sb=new Consulta($sql_sb);
						while($row_sb=$query_sb->ConsultaVerRegistro()){ 
							//$sbprg=table_row($row_sb[id_subprograma],"subprograma");
							
							$gsqla=$gsqlsp." AND aoesp.id_anp_objetivo_especifico= '".$row_sb[id_anp_objetivo_especifico]."' AND m.id_mes='".$_SESSION[idmes]."'";
							$this->setsql($gsqla);
							...
							//TAREA///////////////////////////////////////////////////////////////////////
								$sql_a=$this->set_sql("t.*,aao.*,afao.*,m.*", "$gsqla AND m.id_mes='".$_SESSION[idmes]."'", "t.id_tarea", "aao.nro_asignacion");
								$query_a=new Consulta($sql_a);
								while($row_a=$query_a->ConsultaVerRegistro()){ 
									//$actividad=table_row($row_a[id_actividad],"actividad");
									$gsqlsa=$gsqla."  AND t.id_tarea='".$row_a[id_tarea]."'  AND m.id_mes='".$_SESSION[idmes]."'";
									$this->setsql($gsqlsa);
