...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" id="tabla_despacho" class="gview_frmgrid">
	...
	<tr class="ui-jqgrid-bdiv">
		...
		?>	
			<?	
			if($d>0){
				$area_actual = $doc[$d]->_historial[ count( $doc[$d]->_historial ) - 1 ]['area']->getNombre();
				$area_prev = $doc[$d-1]->_historial[ count( $doc[$d-1]->_historial ) - 1 ]['area']->getNombre();
			}
			
			if( $d == 0 || $area_actual != $area_prev ){ ?>
			<?if ($d >0){?>
			</table>
			<?}?>
			<table class="ui-jqgrid-btable">
				<tr> 
					...
					<?= $doc[$d]->_historial[ count( $doc[$d]->_historial ) - 1 ]['area']->getNombre() ?>
					</th>
				</tr>
			<?}?>
    ...
    function listarDocumentosPorVencer( $usuario ){
        ...
        require("Templates/documentos.php");
    }
	function detalleDocumentosPorUsuario( $usuario,  $documento ){
      ...
      ?>
		...
		<?php
			...
			if($_SESSION['session'][6]&&$estado->getId()!=14&&$estado->getId()!=15)//No esta ni AP ni DR
				$tipo = 1;
		...
		<tr <?=$clase?>>
			...
			<td>
                <div align="center">
                    ...
                    //Archivo Escaneado					
                    if(count($archivo)>0){
						if($his_base[$h]['tipo']==2)
							$ruta = 'Archivados_Respuesta/';
						elseif($his_base[$h]['tipo']==4)
							$ruta = 'Justificados/';	
						else
							$ruta = 'Borradores/';					
					?>					
	...
	function GuardarAprobacion($atencion){
		...
		for($u=0; $u < count($keys); $u++){
			if(!empty($_FILES['doc']['name'][$u])){
				...
				elseif(!move_uploaded_file($_FILES['doc']['tmp_name'][$u], $destino.$_FILES['doc']['name'][$u])){
					...
					$sql = " INSERT INTO aprobacion_escaneado VALUES( '', ".$atencion.",'".$_FILES['doc']['name'][$u]."') ";
					$query = new Consulta($sql);
					echo "se subio el archivo <b>" . $_FILES['doc']['name'][$u] . "</b> satisfactoriamente <br>" ;	
				}	
			}
	...
	function GuardarArchivoRespuesta($archivo){
	
	if($_FILES['respuesta']){
		
		$destino = "Archivados_Respuesta/";
		
		echo  "<div id='error'>";
	
		if(!empty($_FILES['respuesta']['name'])){
			if( $_FILES['respuesta']['size'] > ( 2*1024*1024 ) ){
				echo "El tama�o del archivo supera al permitido";
			}
			elseif(!move_uploaded_file($_FILES['respuesta']['tmp_name'], $destino.$_FILES['respuesta']['name'])){
				echo "ocurrio error al subir archivo " . $_FILES['respuesta']['name'] ;			 				
			}else{
				$sql = " INSERT INTO documento_respuesta_escaneado VALUES( '', ".$archivo.",'".$_FILES['respuesta']['name']."') ";
				$query = new Consulta($sql);
				echo "se subio el archivo <b>" . $_FILES['respuesta']['name'] . "</b> satisfactoriamente <br>" ;	
			}	
		}
		echo "</div>";
	}
	...
	function GuardarAdjuntosArchivo($archivo,$tipo){
	
	if($_FILES['doc']){
		$keys = array_keys($_FILES['doc']);
		$destino = "Adjuntos_Archivo/";
		...
		for($u=0; $u < count($keys); $u++){
			if(!empty($_FILES['doc']['name'][$u])){
				...
				elseif(!move_uploaded_file($_FILES['doc']['tmp_name'][$u], $destino.$_FILES['doc']['name'][$u])){
					...
					$sql = " INSERT INTO adjuntos_archivado VALUES( '', ".$archivo.", ".$tipo.",'".$_FILES['doc']['name'][$u]."') ";
					$query = new Consulta($sql);
					echo "se subio el archivo <b>" . $_FILES['doc']['name'][$u] . "</b> satisfactoriamente <br>" ;	
				}	
			}
