...
if ($ide==''){
   ...
   $query_reg=new Consulta($sql_reg);	
	...
	if($ide=="LT")
        ...
        $where ="AND d.id_estado NOT IN(11) ";
        ...
        if( isset($req->pendientes) ){
            if(isset($req->area) ) $area_session=$req->area;
    ...
    */
    $sql_reg = "SELECT
				...
				WHERE
				(
				(ha.id_area = '".$area_session."' AND ha.tipo_historial_atencion=0) OR
				(ha.id_area_destino = '".$area_session."' AND ha.tipo_historial_atencion=1) OR
                (ha.id_area_destino = '".$area_session."' AND ha.tipo_historial_atencion=2)
				) ".$where."
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" id="tabla_despacho" class="gview_frmgrid">
    ...
    <tr class="ui-widget-content1 jqgrow <?=$clase?> <?=( ( $dias_faltantes > 0 && $vencidos )? 'hidden' : '' )?> ">		
...
function ConsultarDocumento($ids){
    ...
    $sql_dc = "SELECT * FROM documentos_categorias WHERE id_documento = '$ids' ";
  ...
  <table border="0" align="center" bordercolor="#000000" bgcolor="#ffffff" width="100%">
    <tbody>    
      ...
      <tr>
        <td class="Estilo22"><div align="left">Clase de Documento</div></td>
        <td class="Estilo22"><div align="center">:</div></td>
        <td colspan="4" class="Estilo23"><div align="left">
          <?=$remitente_etiquetas[ $dc->categoria ]?>
        </div></td>
      </tr>
      <tr>
