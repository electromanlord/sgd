    ...
    <br/>
        ...
        <div style="float:left; width:33%; text-align:left">
            <fieldset style="width:100%; margin:0px 0px 15px 10px; padding:8px;">
				...
				<input type="hidden" value="<?=$doc->getId()?>" id="id_documento">				
...
function EscaneaGuardar(){ 		 
	...
	$targetPath = $_SERVER['DOCUMENT_ROOT']."/sad/Escaneado_completo/".$doc->getCodigo()."/";

	if ( !is_dir ($targetPath))
		mkdir(str_replace('//','/',$targetPath), 0755);
		
	$keys = array_keys($_FILES['doc']);
	$destino = "../sad/Escaneado_completo/".$doc->getCodigo()."/";
	echo  "<div id='error'>";
		...
		if(!empty($_FILES['doc']['name'][$u])){
			...
			elseif(!move_uploaded_file($_FILES['doc']['tmp_name'][$u], $destino.$_FILES['doc']['name'][$u])){
				...
				$query = new Consulta($sql);
                                    ...
                                    ////////////////////////////  BiG 06/05
                                $sql2 = "INSERT INTO arch_documentos_escaneados_completos
				VALUES('','".$_GET['id']."','".$_FILES['doc']['name'][$u]."')";
...
function EscaneaListado($ide){	  
    ...
    if($ide=='')
		...
		if($ide=='LT')
			...
			if($ide!='LNE') //No escogio Listar por no escaneados
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" id="tabla_despacho" class="gview_frmgrid">
    ...
    <td <?=$tooltip?> width="16%">
      ...
      <td width="28%"><input size="45" value="<?php echo $row_reg["numero_documento"]?>"/></td> 
      <td  width="13%">
	  	<div align="center">
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" id="tabla_despacho" class="gview_frmgrid">
	...
	<tr class="ui-jqgrid-bdiv">
        ...
        <tr class="ui-widget-content1 jqgrow <? echo ($estado==12)?"Estilo2 fila_finalizada":"Estilo2";?>">
            ...
            <td width="28%">
				<input size="43" value="<?=$row_reg["numero_documento"]?>"/>
			</td>
            <td width="13%">
