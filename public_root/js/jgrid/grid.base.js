...
;(function ($) {
/*
 * jqGrid  3.5 alfa - jQuery Grid
 * Copyright (c) 2008, Tony Tomov, tony@trirand.com
 * Dual licensed under the MIT and GPL licenses
 * http://www.opensource.org/licenses/mit-license.php
 * http://www.gnu.org/licenses/gpl.html
 * Date: 2009-03-14 rev 87
 */
$.fn.jqGrid = function( p ) {
	...
	$.fn.setSelection = function(selection,onsr,sd) {
		return this.each(function(){
			...
			if(!$t.p.multiselect) {
				...
				if( $t.p.selrow ) {$("tr#"+$t.p.selrow.replace(".", "\\."),$t.grid.bDiv).removeClass("ui-state-highlight");}
				$t.p.selrow = selection;
				$(pt).addClass("ui-state-highlight");
				...
				if (  ia === -1 ){ 
					if($(pt).attr("class") !== "subgrid") { $(pt).addClass("ui-state-highlight");}
					stat = true;
					$("#jqg_"+$t.p.selrow.replace(".", "\\."),$t.rows).attr("checked",stat);
					$t.p.selarrrow.push($t.p.selrow);
					if( $t.p.onSelectRow && onsr) { $t.p.onSelectRow($t.p.selrow, stat); }
				} else {
					if($(pt).attr("class") !== "subgrid") { $(pt).removeClass("ui-state-highlight");}
					stat = false;
					$("#jqg_"+$t.p.selrow.replace(".", "\\."),$t.rows).attr("checked",stat);
	...
	$.fn.setGridWidth = function(nwidth, shrink) {
		return this.each(function(){
			...
			var initwidth = 0, tblwidth, brd=1, l, vc=0, isSafari,cw,hs=false, scw=1, aw;
			if (!$t.grid ) {return;}
	...
	return this.each( function() {
		...
		$("<div class='loadingui' id=lui_"+this.id+"></div>").insertBefore(this);
		...
		var formatCol = function (elem, pos, row){
		...
		var addCell = function(t,row,cell,pos,irow) {
			var td;
			td = $("<td></td>");
			formatter(td,row,cell,pos,'add');			
			$(row).append(td);
		...
		var formatter = function (elem, row, cellval , colpos, act){
			...
			if(colpos==1)
				...
				asunto = elem[0].textContent || elem[0].innerText;
			else
				elem[0].title = "";
		};
		var addMulti = function(t,row,irow){
			var cbid,td;
			td = $("<td></td>");
			cbid = "jqg_"+row.id;
		...
		var addXmlData = function addXmlData (xml,t, rcnt) {
			...
			var i,fpos,ir=0,v,row,gi=0,si=0,idn, getId,f=[],rd =[], rl= ts.rows.length;
			...
			$(ts.p.xmlReader.root+" "+ts.p.xmlReader.row,xml).each( function( j ) {
				...
				if(ts.p.xmlReader.repeatitems===true){
					$(ts.p.xmlReader.cell,this).each( function (k) {
						...
						addCell(t,row,v,k+gi+si,j+rcnt);
						rd[ts.p.colModel[k+gi+si].name] = v;
					});
				} else {
					for(i = 0; i < f.length;i++) {
						v = $(f[i],this).text();
						addCell(t, row, v , i+gi+si,j+rcnt);
		...
		var addJSONData = function(data,t, rcnt) {
			...
			var ir=0,v,i,j,row,f=[],cur,gi=0,si=0,drows,idn,rd=[], fpos,rl = ts.rows.length;
			...
			for (i=0;i<drows.length;i++) {
				...
				if (ts.p.jsonReader.repeatitems === true) {
					...
					for (j=0;j<cur.length;j++) {
						addCell(t,row,cur[j],j+gi+si,i+rcnt);
					...
					for (j=0;j<f.length;j++) {
						...
						addCell(t,row,v,j+gi+si,i+rcnt);
			...
			if(ts.p.pager) {
				...
				if (ts.p.viewrecords){
					...
					ts.p.reccount == 0 ?
						...
						$(".ui-paging-info",ts.p.pager).html($.jqgformat(ts.p.recordtext,base+1,base+ts.p.reccount,ts.p.records));
				}
		...
		var beginReq = function() {
			...
			switch(ts.p.loadui) {
				case "disable":
					break;
				case "enable":
					$(".loading","#gbox_"+ts.p.id).fadeIn("fast");
					break;
				case "block":
					$("#lui_"+ts.p.id).height(IntNum($(grid.bDiv).height())+IntNum(ts.p._height)+8).fadeIn("fast");
					$(".loading","#gbox_"+ts.p.id).fadeIn("fast");
					break;
			}
		};
		var endReq = function() {
			...
			switch(ts.p.loadui) {
				...
				case "enable":
					...
					break;
				case "block":
					$("#lui_"+ts.id).height(0).fadeOut("fast");
					$(".loading").fadeOut("fast");
					break;
			}
		};
		...
		var setPager = function (){
			...
			pginp = (ts.p.pginput===true) ? "<td>"+$.jqgformat(ts.p.pgtext || "","<input class='ui-pg-input' type='text' size='2' maxlength='7' value='0'/>","<span id='sp_1'></span>")+"</td>" : "",
			pgl="<table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table'><tbody><tr>",
			str, pgcnt, lft, cent, rgt
			clearVals = function(){
				ts.p.selrow = null;
			...
			.append("<div id='"+pgcnt+"' class='ui-pager-control'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:100%;table-layout:fixed;' align='center'><tbody><tr><td id='"+lft+"' align='left'></td><td id='"+cent+"' align='center' style='white-space:nowrap;'></td><td id='"+rgt+"' align='right'></td></tr></tbody></table></div>");
			if(ts.p.pgbuttons===true) {
			...
			var tw = $("<span style='visibility:hidden'></span>").append(pgl).appendTo("#gbox_"+ts.p.id);
			var twd = $(tw).width();
			$(tw).remove();
			...
			$("#first, #prev, #next, #last",ts.p.pager).click( function(e) {
				...
				if(selclick) {
					if ($.isFunction(ts.p.onPaging) ) {ts.p.onPaging(this.id);}
					populate();
					clearVals();
				}
				e.stopPropagation();
		...
		var setColWidth = function () {
			var initwidth = 0, tblwidth, brd=1, l, vc=0,hs,scw=1,cw,hs=true,aw;
			...
			if(isNaN(ts.p.width)) {ts.p.width = initwidth; grid.width =initwidth;}
			...
			if(ts.p.shrinkToFit===true) {
				...
				} else if(grid.width-(initwidth+brd*vc) != 0) {
					cr = grid.width-(initwidth+brd*vc);
				}
				cw= ts.p.colModel[vc].width += cr;
		...
		var getOffset = function (iCol) {
			var i, ret = {};
			ret[0] =  ret[1] = ret[2] = 0;
			for(i=0;i<=iCol;i++){
				if(ts.p.colModel[i].hidden !== true ) {
					ret[0] += ts.p.colModel[i].width+1; //1px is the border (Safari no)
				}
			}
		...
		for(i=0;i<this.p.colNames.length;i++){
			...
			$(th).addClass('ui-widget-header ui-th-column')
		...
		if(ts.p.pager){
			...
			if( $(ts.p.pager).hasClass("scroll")) {
				...
				$(ts.p.pager).appendTo(eg).addClass('ui-widget-header ui-jqgrid-pager');
		...
		var arf = ts.p.hidegrid===true ? $("<a href='#'/>").addClass('ui-jqgrid-titlebar-close').hover(function(){
